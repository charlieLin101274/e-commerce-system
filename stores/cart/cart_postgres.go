...
func (s *PostgresStore) UpsertItem(ctx context.Context, cartID, productID uuid.UUID, quantity int64) (models.CartItem, error) {
	const q = `INSERT INTO cart_items(cart_id,product_id,quantity) VALUES($1,$2,$3) ON CONFLICT(cart_id,product_id) DO UPDATE SET quantity=cart_items.quantity+EXCLUDED.quantity,updated_at=NOW() RETURNING id,cart_id,product_id,quantity,created_at,updated_at`
...
func (s *PostgresStore) UpdateItem(ctx context.Context, cartID, itemID uuid.UUID, quantity int64) (models.CartItem, error) {
	const q = `UPDATE cart_items SET quantity=$3,updated_at=NOW() WHERE cart_id=$1 AND id=$2 RETURNING id,cart_id,product_id,quantity,created_at,updated_at`
...
func (s *PostgresStore) DeleteItem(ctx context.Context, cartID, itemID uuid.UUID) error {
	tag, err := s.db.Exec(ctx, `DELETE FROM cart_items WHERE cart_id=$1 AND id=$2`, cartID, itemID)
	if err != nil {
		return err
	}
