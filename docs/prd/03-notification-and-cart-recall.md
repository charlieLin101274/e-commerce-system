...
MVP 將 Cart Recall Trigger 與 Notification Delivery 實作為可獨立執行的 worker/component，先保留在同一 repository，不要求拆成獨立 microservice。API Server 只在 Cart transaction 寫入 PostgreSQL Outbox event。Trigger worker 直接 polling Outbox、執行 inbox deduplication、建立 Journey、執行 Rule evaluation，最後建立 Notification Task。External queue 是正式流量增加後的演進方向，不是七天 MVP 的交付範圍。
...
- Prometheus、OpenTelemetry、rate limiting 與完整 load test 已列入 Production Readiness Backlog，不屬於七天 MVP acceptance criteria。MVP 以 unit tests、integration tests、SQL constraints 與 structured logs 驗證核心流程。
